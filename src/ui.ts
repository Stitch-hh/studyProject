  ...
  scr.append(
    ...
    el(
      ...
      'Каждое вмешательство Света даёт Тьме право на ответ той же силы. ' +
        ...
        'Смена начинается в 22:00 и закончится в 06:00, чего бы это ни стоило.',
    ),
  );
  ...
  if (inc.witness) {
    card.append(
      el(
        'div',
        'complication',
        'Осложнение: рядом Спящий — любое грубое вмешательство он увидит.',
      ),
    );
  }
  ...
  card.append(
    ...
    el(
      ...
      state.log[state.log.length - 1] +
        '<br><br>Спящий стоит в десяти шагах и смотрит на вас круглыми глазами.',
    ),
  );
  const opts = el('div', 'options');
  ...
  if (report.arbitration) {
    totals.append(
      el(
        'p',
        'card-text warn',
        'Арбитраж уведомляет: перекос Баланса зафиксирован. Ещё одна такая ночь — и в город войдёт Зеркало.',
      ),
    );
  }
  scr.append(totals);
